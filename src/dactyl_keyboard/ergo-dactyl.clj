(ns dactyl-keyboard.ergo-dactyl
...
(defn key-place [column row shape]
  (let [row-placed-shape (->> shape
                              ...
                              (translate [0 0 row-radius]))
        column-offset (cond
                        (= column 3) [0 2.82 -3.0] ;;was moved -4.5
                        (or (= column 5) (and (= column 6) (= row 4))) [0 -5.8 5.64]
                        ;; This is column 5, rows 0-3, which need 1.5u room for
                        ;; Ergodox cap compatibility
                        (= column 6) [5 -5.8 6.64]
                        :else [0 0 0])
        column-angle (* β (- 3 column))
...
(def key-holes
  (let [one-us (for [column columns
         ...
         ;; Actually "one-and-a-halfs", for col6, rows 0-3
  ...
  (apply union
         (for [column columns
          ...
          (->> (sa-cap
                 ...
                 (if (and (= column 6) (not= row 4)) 1.5 1)
                 )
                (key-place column row)))))
...
;; In column units
;; Add 0.05 column units to the outside wall to handle 1.5u key caps
(def right-wall-column (+ (last columns) 0.05))
;; JD 3/2 will move wall, but need to move column
(def left-wall-column (- (first columns) 3/2))
  ...
  (let [step wall-step
    ...
    (union
     ...
     (hull (place 5 0 (translate [0 -1 1] wall-sphere-bottom-back))
           ...
           (key-place 6 0 web-post-tl)
           (key-place 6 0 web-post-tr))
             ...
             (for [x (range 0 5)]
               (union
                (hull (place right-wall-column x (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (key-place 6 x web-post-br)
                      (key-place 6 x web-post-tr))))
             (for [x (range 0 4)]
               (union
                (hull (place right-wall-column x (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (place right-wall-column (inc x) (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (key-place 6 x web-post-br)
                      (key-place 6 (inc x) web-post-tr))))
             [(union
   ...
   (let [shift #(translate [0 0 (+ (- web-thickness) -5)] %)
         ...
         front-wall (concat
                     ...
                     [(hull (case-place right-wall-column 4 (translate [0 1 1] wall-sphere-bottom-front))
                            ...
                            (key-place 6 4 half-post-bl)
                            (key-place 6 4 half-post-br))
         ...
         thumb-inside [(triangle-hulls
                       ...
                       (hull
                        ...
                        (key-place 2 4 (translate [0 0 8.5] web-post-bl))
                        (key-place 2 4 half-post-bl)
                        )]
         stands (let [bumper-diameter 9.6
                  ...
                  [(stand-at #(key-place 0 1 %))
                   ...
                   (stand-at #(key-place 6 0 %))
                   (stand-at #(key-place 6 3 %))])]
     (apply union
            (concat
             main-keys-bottom
             front-wall
...
(defn circuit-cover [width length height]
  (let [cover-sphere-radius 1
        ...
        bl (->> cover-sphere lower-to-bottom (key-place 1 1/2))
        br (->> cover-sphere lower-to-bottom (key-place 2 1/2))
        tl (->> cover-sphere lower-to-bottom (key-place 1 5/2))
        tr (->> cover-sphere lower-to-bottom (key-place 2 5/2))
...
(spit "things/ergo-switch-hole.scad"
      (write-scad single-plate))

(spit "things/ergo-holes.scad"
      (write-scad (union connectors key-holes)))

(spit "things/ergo-dactyl-top-right.scad"
      (write-scad dactyl-top-right))

(spit "things/ergo-dactyl-bottom-right.scad"
      (write-scad dactyl-bottom-right))

(spit "things/ergo-dactyl.scad"
...
(spit "things/ergo-dactyl-bottom-left.scad"
      (write-scad dactyl-bottom-left))

(spit "things/ergo-dactyl-top-left-with-teensy.scad"
      (write-scad (mirror [-1 0 0] dactyl-top-right)))

(spit "things/ergo-dactyl-bottom-left-with-teensy.scad"
